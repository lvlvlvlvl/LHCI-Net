...
from ultralytics.nn.extra_modules.block import IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, \
    ...
    CSP_PMSFA,HyperComputeModule,MANet
...
from ultralytics.nn.extra_modules.transformer import TransformerEncoderLayer_Pola_FMFFN
class HiddenPrints:
    def __enter__(self):
...
def get_pruner(opt, model, example_inputs):
    ...
    #     if isinstance(m, RTDETRDecoder):
    #         ignored_layers.append(m)
    #     if isinstance(m, CSP_PMSFA):
    #         ignored_layers.append(m)


    # ultralytics/cfg/models/rt-detr/rtdetr-CSP-PMSFA+rtdetr-Pola-FMFFN.yaml
    customized_pruners[RepConv] = RepConvPruner()
    for k, m in model.named_modules():
        if isinstance(m, AIFI):
            ignored_layers.append(m)
        ...
        if isinstance(m, CSP_PMSFA):
            ignored_layers.append(m)
        if isinstance(m, TransformerEncoderLayer_Pola_FMFFN):
            ignored_layers.append(m)
        ...
        if isinstance(m, HyperComputeModule):
            ignored_layers.append(m)
    #TransformerEncoderLayer_Pola_FMFFN要跳过的层
    ignored_layers.append(model.model[11])
    ignored_layers.append(model.model[16])
